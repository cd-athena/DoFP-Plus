...
#define N_REP 1 /* Number of available media representations (quality levels) */
#define N_MAX_SEG 5 /* Max number of segments to be downloaded */
...
/* NEW VARS */
static long double      download_time[N_MAX_SEG] = {0.0}; /* Downlaod time for the segments (counted from the beginning) */
static unsigned         unused_segments = 0U;
/* OLD VARS */
static long double 		throughput;
static int 				add_requests = 1;
static unsigned			experiment_setup = 1; /* 1 -> No PUSH, 2 -> A FIRST - N LAST, 3 -> C FIRST - N LAST, 4 -> N FIRST - C LAST, 5 -> N FIRST - A LAST, 6 -> CONCURRENT */
static unsigned 		urgency[N_MAX_SEG] = {1,2,3,4,5};
static unsigned 		urgency_ind = 0;
static unsigned			incremental = 0;
static double const 	buffer_size = 20.0; // Maximum buffer size
static double 			buffer_level = 2.0; /* Updated buffer level of the downloaded segment */
static unsigned 		seg_length = 4U; /* Lenght of the segments to be downloaded */
static unsigned 		playout = 1U; /* States whether the playout is running [1] or is paused (stall or buffering) [0] */
...
http_client_on_new_stream (void *stream_if_ctx, lsquic_stream_t *stream)
{
	...
	st_h->sh_seg_ind = seg_ind;
	seg_ind++;
	printf("\n!! Stream carrying segment %u with urgency %u created at time: %.3Lf !!\n", st_h->sh_seg_ind, urgency[urgency_ind], (long double) st_h->sh_created);
	...
	lsquic_stream_set_http_prio(stream,
                &(struct lsquic_ext_http_prio){
                    .urgency = urgency[urgency_ind],
                    .incremental = incremental,
                }
    );
	
	st_h->sh_urgency = urgency[urgency_ind];
...
http_client_on_close (lsquic_stream_t *stream, lsquic_stream_ctx_t *st_h)
{
	...
	// Update buffer when segment has been downloaded
	//update_buff(true);
	...
	printf("\n!! Stream with urgency %u closed at time: %.3Lf !!\n", st_h->sh_urgency, (long double) lsquic_time_now());
	download_time[st_h->sh_seg_ind - 1] = (long double) (lsquic_time_now() - start_t) / 1000000;
	long double deadline = (long double) 2 + (st_h->sh_seg_ind - 2) * seg_length;
	printf("\n!! Download time from beginning of the playout: %.3Lf s !!\n", download_time[st_h->sh_seg_ind - 1]);
	if (download_time[st_h->sh_seg_ind - 1] > deadline) {
		printf("Download time (%.3Lf [s]) > Deadline (%.3Lf [s]) -> Segment unused\n", download_time[st_h->sh_seg_ind - 1], deadline);
		unused_segments++;
	} else {
		printf("Download time (%.3Lf [s]) <= Deadline (%.3Lf [s]) -> Segment used\n", download_time[st_h->sh_seg_ind - 1], deadline);
	}
	
	st_h->sh_throughput = (long double) st_h->sh_nread * 8 / ((long double) 1000 * (lsquic_time_now() - st_h->sh_created) / 1000000); // [kbps]
	printf("!! Throughput for stream: %.3Lf !!\n",st_h->sh_throughput);
	
	// printf("\nConn_n_requests: %d\n", conn_h->ch_n_reqs);
	// printf("Total requests: %d, Requests per connection: %d\n", client_ctx->hcc_total_n_reqs, client_ctx->hcc_reqs_per_conn);
	...
	if (add_requests) {
		...
		for (int i = 0; i < 4; i++) {
			...
			pe->path = "/file-512K"; /* Path of the next requested segment */
			TAILQ_INSERT_TAIL(&client_ctx->hcc_path_elems, pe, next_pe);
			increment = MIN(client_ctx->hcc_total_n_reqs, client_ctx->hcc_reqs_per_conn);
			conn_h->ch_n_reqs += increment;
			client_ctx->hcc_total_n_reqs -= increment;
		}
		buffer_level = 2.0;
    ...
    {
		printf("\nAll requests completed, closing connection!\n");
...
main (int argc, char **argv)
{
    ...
    while (-1 != (opt = getopt(argc, argv, PROG_OPTS
                                    "46Br:R:IKu:EP:M:n:w:H:p:0:q:e:hatT:b:d:x:"
	...
	/* SETUP EXPERIMENT */
	/* 1 -> No PUSH, 2 -> A FIRST - N LAST, 3 -> C FIRST - N LAST, 4 -> N FIRST - C LAST, 5 -> N FIRST - A LAST, 6 -> CONCURRENT */
	if (experiment_setup == 1) { // NO PUSH
		incremental = 0;
		client_ctx.hcc_total_n_reqs = 5;
		for (unsigned i = 0; i < N_MAX_SEG; i++) {
			urgency[i] = i + 1;
		}
		client_ctx.hcc_cc_reqs_per_conn = 1;
	} else if (experiment_setup == 2) { // A FIRST - N LAST
		incremental = 0;
		client_ctx.hcc_total_n_reqs = 5;
		for (unsigned i = 0; i < N_MAX_SEG; i++) {
			urgency[i] = i + 1;
		}
		client_ctx.hcc_cc_reqs_per_conn = 4;
	} else if (experiment_setup == 3) { // C FIRST - N LAST
		incremental = 0;
		client_ctx.hcc_total_n_reqs = 5;
		for (unsigned i = 0; i < N_MAX_SEG - 1; i++) {
			urgency[N_MAX_SEG - 2 - i] = i + 1;
		}
		urgency[N_MAX_SEG - 1] = N_MAX_SEG;
		client_ctx.hcc_cc_reqs_per_conn = 4;
	} else if (experiment_setup == 4) { // N FIRST - C LAST
		incremental = 0;
		client_ctx.hcc_total_n_reqs = 5;
		urgency[N_MAX_SEG - 1] = 1;
		for (unsigned i = 0; i < N_MAX_SEG - 1; i++) {
			urgency[i] = i + 2;
		}
		client_ctx.hcc_cc_reqs_per_conn = 4;
	} else if (experiment_setup == 5) { // N FIRST - A LAST
		incremental = 0;
		client_ctx.hcc_total_n_reqs = 5;
		for (unsigned i = 0; i < N_MAX_SEG; i++) {
			urgency[N_MAX_SEG - 1 - i] = i + 1;
		}
		client_ctx.hcc_cc_reqs_per_conn = 4;
	} else { // CONCURRENT
		incremental = 1;
    ...
    if (stats_fh)
    {
        elapsed = (long double) (lsquic_time_now() - start_t) / 1000000;
	...
	printf("\n !!STATS!!\n");
	printf("Download time for the segments: ");
	for (int i = 0; i < N_MAX_SEG; i++) {
		printf("[%.3Lf] ", download_time[i]);
	}
	printf("\nUnused segments: %u\n", unused_segments);
